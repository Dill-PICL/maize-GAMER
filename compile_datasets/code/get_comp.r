...
compile_comprehensive = function(datasets,obo){
    #data_dir="nr_test"
    #obo="obo/go.obo"
...
write_oufile <- function(out_gaf){
    
    outfile = "comp_data/maize_v3.agg.gaf"
    cat("Writing Complied Comprehensive Data to",outfile,"\n")
    out_gaf$assigned_by = "Aggregate"
    write_gaf(out_gaf,outfile)
    
    nr_outfile = "nr_data/maize_v3.agg.gaf"
    ...
    cat("Writing Complied Non-Redundant Comprehensive Data to",nr_outfile,"\n")
    out_gaf$assigned_by = "Aggregate"
    write_gaf(comp_gaf,nr_outfile)
}
