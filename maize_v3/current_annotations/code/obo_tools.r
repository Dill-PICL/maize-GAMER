...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
# Functions in this section are to read and fix GO OBO files input
...
get_aspect = function(obo_data,go_ids){
    ns2aspect=list(molecular_function="F",biological_process="P",cellular_component="C")
    #go_ids = gaf_data$term_accession
    tmp_go_ids = unique(go_ids)
    
    go_ns = obo_data$namespace[tmp_go_ids]
    go_ns2 = lapply(go_ns,function(x){
        if(is.null(x)){
            list(x="N")
        }else{
            ns2aspect[[x]]
    ...
    names(go_ns2) = tmp_go_ids
    tmp_out = go_ns2[go_ids]
    return(unlist(tmp_out))
}
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
#functions in this section are to plot GO graphs for different annotations
...
plot_go_illus = function(gene,gold_terms,tool_term,dot_file,title){
    ns = unique(unlist(go_obo$namespace[tool_term]))
    #rel_colors = list(is_a="blue",part_of="red")
    rel_colors = list(is_a="black",part_of="black")
    cat("digraph G{\n",file = dot_file)
    cat("\trankdir = BT\n",file = dot_file,append=T)
...
plot_example_fig = function(gold,tool_data,dot_file,title){
    gene=unique(tool_data$db_object_symbol)
    tmp_aspect=unique(tool_data$aspect)
    gold_terms = gold[db_object_symbol == gene & aspect == tmp_aspect]$term_accession
